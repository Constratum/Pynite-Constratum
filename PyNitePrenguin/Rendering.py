...
class Renderer:
    ...
    def plot_pt_load(
        ...
        direction: Union[Tuple[float, float, float], np.ndarray],
        length: float,
        label_text: Optional[Union[str, float, int]] = None,
        color: str = "green",
    ) -> None:
    ...
    def plot_dist_load(
        ...
        direction: Union[np.ndarray, Tuple[float, float, float]],
        length1: float,
        length2: float,
        label_text1: Optional[Union[str, float, int]],
        label_text2: Optional[Union[str, float, int]],
        color: str = "green",
    ) -> None:
                ...
                if load[2] in load_factors:
                    ...
                    if load[0] in {"FX", "FY", "FZ"}:
                        self.plot_pt_load(
                            ...
                            direction,
                            abs(load_value / max_pt_load) * 1.5 * self.annotation_size,
                            load_value,
                            "green",
                        )
                        ...
                        self.plot_moment(
                            (node.X, node.Y, node.Z),
                            direction,
                            abs(load_value / max_moment) * 1.0 * self.annotation_size,
                            str(load_value),
                            "green",
                        )
                ...
                if load[3] in load_factors:
                    ...
                    if load[0] == "Fx":
                        self.plot_pt_load(
                            position,
                            dir_cos[0, :],
                            load_value / max_pt_load * 1.5 * self.annotation_size,
                            load_value,
                        )
                    elif load[0] == "Fy":
                        self.plot_pt_load(
                ...
                if load[5] in load_factors:
                    ...
                    if load[0] in {"Fx", "Fy", "Fz", "FX", "FY", "FZ"}:
                        ...
                        self.plot_dist_load(
                            ...
                            position2,
                            direction,
                            w1 / max_dist_load * 1.5 * self.annotation_size,
                            w2 / max_dist_load * 1.5 * self.annotation_size,
                            str(round(w1, 1)),
                            str(round(w2, 1)),
                            "green",
                        )
            ...
            for load in plate.pressures:
                ...
                if load[1] in load_factors:
                    ...
                    self.plot_area_load(
                        ...
                        position3,
                        dir_cos * sign,
                        load_value / max_area_load * 1.5 * self.annotation_size,
                        str(round(load_value, 1)),
                        color="green",
                    )
