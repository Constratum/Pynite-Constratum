...
#%%
    def __init__(self, name: str, i_node: Node3D, j_node: Node3D, ks: Union[float, UniaxialMaterial],
                 LoadCombos: Dict[str, LoadCombo] = {'Combo 1': LoadCombo('Combo 1', factors={'Case 1': 1.0})},
                 tension_only: bool = False, comp_only: bool = False,
                 spring_type: str = 'axial') -> None:
        '''
        Initializes a new spring.

        `ks` may be a linear spring constant (float), or a `UniaxialMaterial` for a nonlinear
        spring. `spring_type` selects which local DOF the spring acts on - 'axial' or
        'rotational_z'.
        '''
        ...
        if isinstance(ks, (int, float)):
            # Linear spring
            self.ks: float = float(ks)   # The spring constant (force/displacement)
            self.material: Optional[UniaxialMaterial] = None
            self.is_nonlinear: bool = False
        else:
            # Nonlinear spring driven by a uniaxial material. The initial tangent is kept as the
            # reference stiffness so linear-only code paths still have a usable `ks`.
            self.ks: float = ks.get_initial_tangent()
            self.material: Optional[UniaxialMaterial] = ks
        ...
        # For nonlinear springs, store the current deformation state
...
#%%
    def ke(self) -> NDArray[float64]:
        ...
        For nonlinear springs this returns the tangent stiffness matrix based on the current
        ...
        if self.spring_type == 'rotational_z':
            # Rotational spring about the local Z-axis
            ke = array([
...
#%%   
    def f(self, combo_name: str = 'Combo 1') -> NDArray[float64]:
        ...
        For nonlinear springs this uses the material stress-strain relationship. For linear
        springs this uses the standard stiffness relationship.
        ...
        if self.is_nonlinear and self.material is not None:
            ...
            # For nonlinear materials, calculate the force from the material stress. Get the
            # current deformation first.
            d_local = self.d(combo_name)

            if self.spring_type == 'rotational_z':
                # Relative rotation between the j and i nodes
                deformation = d_local[11, 0] - d_local[5, 0]
            else:
                # Relative axial displacement between the j and i nodes
                deformation = d_local[6, 0] - d_local[0, 0]
            ...
            # Build the force vector based on the spring type
            if self.spring_type == 'rotational_z':
                # Rotational spring - moment about the local Z-axis
                f = array([[0], [0], [0], [0], [0], [-stress],
                           [0], [0], [0], [0], [0], [stress]])
            else:
                # Axial spring - force along the local X-axis
                f = array([[-stress], [0], [0], [0], [0], [0],
                           [stress], [0], [0], [0], [0], [0]])
...
#%%
    def T(self) -> NDArray[float64]:
        ...
        # Handle zero-length springs, which are common for rotational springs and connections
        # modelled between two coincident nodes. There is no geometry to derive an orientation
        # from, so the local axes are taken as parallel to the global axes.
...
#%%
    def D(self, combo_name: str = 'Combo 1') -> NDArray[float64]:
        ...
        if self.spring_type == 'rotational_z':
            ...
            # For rotational springs, only apply rotational displacements if the spring is active
            ...
            # For axial springs, only apply axial displacements if the spring is active
            if self.active[combo_name] == True:
                D[0, 0] = self.i_node.DX[combo_name]
                D[6, 0] = self.j_node.DX[combo_name]
...
#%%
    def set_trial_deformation(self, combo_name: str = 'Combo 1') -> None:
        '''
        Sets the trial deformation for nonlinear springs. This method should be called during
        the analysis iteration process.
...
#%%
    def commit_state(self) -> None:
        '''
        Commits the current trial state for nonlinear springs. This method should be called
        after convergence is achieved.
        '''
...
#%%
    def revert_to_last_commit(self) -> None:
        '''
        Reverts the trial state to the last committed state for nonlinear springs. This method
        should be called when iterations fail to converge.
        '''
