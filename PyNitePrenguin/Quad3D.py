...
import numpy as np
from numpy import add, array, cross, zeros
...
class Quad3D:
    ...
    def __init__(
        ...
        self.pressures: List[Tuple[float, str]] = (
            []
    ...
    def M(self) -> NDArray[float64]:
        """Returns the quad element's global mass matrix using consistent mass matrix approach.

        :return: The global mass matrix for the quad element.
        :rtype: array
        """
        # Get material properties
        rho = (
            self.model.materials[self.material_name].rho
            if hasattr(self, "material_name")
            else 2400.0
        )  # Default concrete density

        # For a quad element, we need to integrate the mass matrix over the element area
        # Using a simplified approach based on the element area and thickness

        # Calculate element area (approximate for general quadrilateral)
        # Using the cross product method for the area of a quadrilateral
        # Split into two triangles and sum their areas

        # Vectors from node i to nodes j and n
        v1 = array(
            [
                self.j_node.X - self.i_node.X,
                self.j_node.Y - self.i_node.Y,
                self.j_node.Z - self.i_node.Z,
            ]
        )
        v2 = array(
            [
                self.n_node.X - self.i_node.X,
                self.n_node.Y - self.i_node.Y,
                self.n_node.Z - self.i_node.Z,
            ]
        )
        area1 = 0.5 * norm(cross(v1, v2))

        # Vectors from node m to nodes j and n
        v3 = array(
            [
                self.j_node.X - self.m_node.X,
                self.j_node.Y - self.m_node.Y,
                self.j_node.Z - self.m_node.Z,
            ]
        )
        v4 = array(
            [
                self.n_node.X - self.m_node.X,
                self.n_node.Y - self.m_node.Y,
                self.n_node.Z - self.m_node.Z,
            ]
        )
        area2 = 0.5 * norm(cross(v3, v4))

        total_area = area1 + area2

        # Calculate total mass of the element
        total_mass = rho * total_area * self.t

        # Create a consistent mass matrix for a rectangular plate element
        # This is a simplified approach - for more accuracy, numerical integration would be needed

        # Mass per node (assuming uniform distribution)
        mass_per_node = total_mass / 4.0

        # Rotational inertia terms (simplified)
        rot_inertia = total_mass * (self.t**2) / 12.0 / 4.0  # Distributed among nodes

        # Initialize mass matrix (24x24 for 4 nodes with 6 DOF each)
        m = zeros((24, 24))
        ...
        for i in range(4):
            ...
            m[base_idx + 0, base_idx + 0] = mass_per_node  # X translation
            m[base_idx + 1, base_idx + 1] = mass_per_node  # Y translation
            m[base_idx + 2, base_idx + 2] = mass_per_node  # Z translation

            # Rotational mass terms (simplified)
            m[base_idx + 3, base_idx + 3] = rot_inertia  # X rotation
            m[base_idx + 4, base_idx + 4] = rot_inertia  # Y rotation
            m[base_idx + 5, base_idx + 5] = rot_inertia  # Z rotation
